...
| `python main.py panorama "<场景描述或 ENV_00X>"` | **场景 360° 全景空间基准**（§4.6「全景定基准 → 六角度出分镜可用图」） |
...
| 场景 S01–S06 六角度出图 | ⚠️ 全景基准已就位，六角度按 `INDEX-TEMPLATES.md` §4.1 手工取用 |
| 视觉层一致性比对（图与图） | ❌ 未实现（现为**结构化**指纹锁定，非图像比对） |
