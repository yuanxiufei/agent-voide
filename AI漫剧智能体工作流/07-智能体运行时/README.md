...
> ⚠️ **§六 还有一项本项目无法判定，`drift` 会如实列出而不是假装比过**：
> 它要求比对「风格锚点表」，而该表模板指向
> `02-服化道/引擎/TURNAROUND-STANDARD.md` **§七** —— 但 §七 的实际内容是
> 「光影设计的三个来源与分工」，**不含风格锚点表**。即 §六 的这条交叉引用**指错了**，
> 本项目也从未产出过该表 → **无可比对之表**。
...
| **§六 漂移检测**（ID 是否都在总表、是否自造 ID） | ✅ 已实现（`drift`）—— **24 项测试**（`tests/test_drift.py`，含**故意注入的漂移**）|
| §六 的「风格锚点表」比对 | ❌ 无法判定 —— **§六 指向的表不存在**（§七 内容是光影设计），已如实列出 |
