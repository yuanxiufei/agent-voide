...
> ⚠️ **仍有一处工作流的错引用待订正**：§六（以及根 README 的对照表）把风格锚点表
> 指向 `TURNAROUND-STANDARD.md` §七。运行时的判定已不依赖它，但**文档本身仍需修**
> （`drift` 会一直把这条列在「未核验/待办」里，不会因为"能跑"就不提）。
...
| §六 指向的「风格锚点表」 | ⚠️ **该表不存在**（§六 指到 `TURNAROUND-STANDARD.md` §七，而 §七 是光影设计）→ 运行时改为**按权威规则里的锚点**判定，已能查画风漂移；**工作流文档的错引用仍待订正** |
