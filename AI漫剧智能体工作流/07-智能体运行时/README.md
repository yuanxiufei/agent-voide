...
| 查询的**过滤词** | **8 个写死的词**（机械/废土/银发…），抽不到就**静默列出全部**，且 CLI **不显示过滤条件** | **用户原话**（按结构抽宾语 + 去尾部类目词）→ **词表增强**（226 个已知值）；抽不到时**明说**「未加过滤」 |
...
**查询同理**（以下为实测输出）—— 以前只有 `router.QUERY_WORDS` 里那几个词能当过滤条件，
**抽不到就静默列出全部**，而且 CLI 连过滤条件都不显示：

```bash
$ python main.py "有哪些旗袍"
  📚 查询结果：1 项
  过滤词「旗袍」（取自你的原话）            ← 新增：过滤条件**必须显示**
  CHR_001  民国女特工-民国  [character]  民国  v001  DRAFT

$ python main.py "列出所有"                  # 没写对象
  📚 查询结果：1 项
  未从输入里识别出过滤词 —— 本次是**按类型列出全部**；想过滤请写明对象…

$ python main.py "有哪些银发角色"             # 库里没有 → 0 项，但过滤词照样显示
  📚 查询结果：0 项
  过滤词「银发」（取自你的原话）
```

> ⚠️ 同一轮端到端**还抓出一个更严重的**：`查看 CHR_001` 被路由判成 **create**，
> **真的建了一张名叫「查看 CHR_001」的垃圾资产** —— 因为 `router.QUERY_WORDS` 里
> 没有「查看」。修法**不是补一个词**，而是让它与抽取**共用同一张表**
> （`agent._QUERY_WORDS = router.QUERY_WORDS + 所有/全部`），
> 并把测试从「抽取层」提到**路由层**（抽取单测全绿也拦不住路由判错）：
