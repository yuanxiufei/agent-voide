...
MECHANISM_RULES = [
    ...
    # ── §六 漂移检测（2026-09-24：原文里"该由代码做"的机制，此前只是段文字）──
    {
        "name": "07 运行时：§六 漂移检测**已实现**（不是只粘进 System Prompt）",
        "keyword": "这是机制，不是规则",
        "must_in": ["07-智能体运行时/README.md"],
    },
    {
        "name": "07 运行时：外来前缀不判定（SHT_/VID_/AUD_ 归别的模块，防误报）",
        "keyword": "本项目**无总表 → 不判定**",
        "must_in": ["07-智能体运行时/README.md"],
    },
    {
        "name": "07 运行时：§六 风格锚点表**指错了**这一事实必须写明",
        "keyword": "指错了",
        "must_in": ["07-智能体运行时/README.md"],
    },
    {
        "name": "07 运行时：漂移检测有测试且**故意注入漂移**",
        "keyword": "故意注入的漂移",
        "must_in": ["07-智能体运行时/README.md"],
    },
    {
        "name": "07 运行时：**风格锚点漂移**也在检测范围内（防画风漂移）",
        "keyword": "风格锚点漂移",
        "must_in": ["07-智能体运行时/README.md"],
    },
    {
        "name": "07 运行时：风格锚点判定不依赖那张不存在的表",
        "keyword": "不依赖那张表",
        "must_in": ["07-智能体运行时/README.md"],
    },
    ...
    # ── 通用性：任何一部小说都能跑（2026-09-24；此前绑死固定题材表）──
    {
        "name": "07 运行时：通用性三原则（用户原话 > 预设；按语法抽词；抽不到如实标注）",
        "keyword": "按「语法」抽词，不按「词表」",
        "must_in": ["07-智能体运行时/README.md"],
    },
    {
        "name": "07 运行时：预设**仅在真命中时**才用（否则假具体值会覆盖用户原话）",
        "keyword": "仅当真命中时",
        "must_in": ["07-智能体运行时/README.md"],
    },
    {
        "name": "07 运行时：通用性有测试（含「英文纯度」断言）",
    ...
    # ── 查询的过滤词与路由（2026-09-26）──
    {
        "name": "07 运行时：查询过滤词是**结构抽取**（不再是 8 个写死的词）",
        "keyword": "过滤词「旗袍」（取自你的原话）",
        "must_in": ["07-智能体运行时/README.md"],
    },
    {
        "name": "07 运行时：抽取与路由**共用同一张查询动词表**（防两处漂移）",
        "keyword": "router.QUERY_WORDS",
        "must_in": ["07-智能体运行时/README.md"],
    },
    {
        "name": "07 运行时：查询的**过滤条件必须可见**（不静默列全部）",
        "keyword": "未从输入里识别出过滤词",
        "must_in": ["07-智能体运行时/README.md"],
    },
    # ── 类型判定去词表化（2026-09-24）──
    {
        "name": "07 运行时：类型判定「能靠语法判的，绝不靠名词表」",
        "keyword": "能靠语法判的，绝不靠名词表",
        "must_in": ["07-智能体运行时/README.md"],
    },
    {
        "name": "07 运行时：两条语法事实（中心语在最后 / 把字句是介词）",
        "keyword": "汉语名词短语的中心语在最后",
        "must_in": ["07-智能体运行时/README.md"],
    },
    {
        "name": "07 运行时：已知歧义如实记录（不写进测试当唯一答案）",
        "keyword": "已知歧义",
        "must_in": ["07-智能体运行时/README.md"],
    },
    ...
    {
        # ⭐ 权威边界必须写着：`智能体搭建参考md/` 是**对外迁移配置**，
        #   本仓库内以工作流为准；两者**不自动同步**。
        #   不写下来，后来人会以为它是"第二权威"（实测差点如此）。
        "name": "07 运行时：参考 md 的定位（对外迁移配置）与权威边界要写明",
        "keyword": "对外迁移配置",
        "must_in": ["07-智能体运行时/README.md"],
    },
